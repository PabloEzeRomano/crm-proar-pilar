    ...
    (set, get) => ({
      ...
      fetchMoreVisits: async () => {
        ...
        const { data, error } = await supabase
          ...
          .or(
            `scheduled_at.lt.${oldest.scheduled_at},and(scheduled_at.eq.${oldest.scheduled_at},id.lt.${oldest.id})`
          )
          .limit(PAGE_SIZE);
      ...
      createVisit: async (data: CreateVisitInput) => {
        ...
        const isCreatedInTheFuture = dayjs(newVisit.scheduled_at).isAfter(
          dayjs()
        );
      ...
      updateVisit: async (id: string, data: UpdateVisitInput) => {
        ...
        if (
          existingVisit &&
          data.scheduled_at &&
          existingVisit.scheduled_at !== data.scheduled_at
        ) {
          try {
            ...
            const isScheduledInTheFuture = dayjs(
              updatedVisit.scheduled_at
            ).isAfter(dayjs());
            if (isScheduledInTheFuture && updatedVisit.client) {
              ...
              if (notificationId) {
                ...
                if (updateError) {
                  console.error(
                    'Failed to save new notification_id:',
                    updateError
                  );
                } else {
