...
test.describe('Visual Audit (EP-051.2)', () => {
  ...
  test('screenshot all main screens', async ({ page }) => {
    ...
    async function auditScreen(
      ...
      expect(
        bodyText,
        `${label}: literal "undefined" in page text`
      ).not.toMatch(/\bundefined\b/);
      // "null" is a common Spanish word fragment; check only as standalone token.
      expect(
        bodyText,
        `${label}: literal standalone "null" in page text`
      ).not.toMatch(/(?<![a-z])null(?![a-z])/i);
    ...
    const firstClientBtn = page
      ...
      .first();
    const clientBtnVisible = await firstClientBtn
      .isVisible()
      .catch(() => false);
    if (clientBtnVisible) {
    ...
    const firstVisitBtn = page
      .locator('[role="button"][aria-label*="Ver visita"]')
      .first();
