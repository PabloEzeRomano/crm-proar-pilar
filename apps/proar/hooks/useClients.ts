...
export function useClients(
  ...
  const fetchAllClientsForAdmin = useClientsStore(
    (state) => state.fetchAllClientsForAdmin
  );
  ...
  const filteredClients = useMemo<Client[]>(() => {
    ...
    if (rubroFilter?.length) {
      result = result.filter(
        (c) => c.industry && rubroFilter.includes(c.industry)
      );
    }
    ...
    switch (sortOrder) {
      ...
      case 'stale-first':
        sorted.sort((a, b) => {
          const aDays = a.last_visited_at
            ? dayjs().diff(dayjs(a.last_visited_at), 'day')
            : Infinity;
          const bDays = b.last_visited_at
            ? dayjs().diff(dayjs(b.last_visited_at), 'day')
            : Infinity;
          return bDays - aDays;
        });
        break;
  ...
  return {
    ...
    error,
    fetchClients: () =>
      isAdminMode ? fetchAllClientsForAdmin() : fetchClientsAction(),
    createClient,
    updateClient,
    deleteClient,
    ownerProfiles,
