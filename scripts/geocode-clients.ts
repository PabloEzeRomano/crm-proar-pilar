...
async function geocodeClients() {
  ...
  for (let i = 0; i < clients.length; i++) {
    ...
    if (!client.address || !client.city) {
      console.log(`[${i + 1}/${clients.length}] ⏭️  ${client.name} — no address/city, skipping`)
      failed++
      continue
    }

    try {
      const query = encodeURIComponent(`${client.address}, ${client.city}, Argentina`)
      ...
      if (!response.ok) {
        console.log(`[${i + 1}/${clients.length}] ⚠️  ${client.name} — HTTP ${response.status}`)
        failed++
        await sleep(1100)
        continue
      }
      ...
      if (!results || results.length === 0) {
        console.log(`[${i + 1}/${clients.length}] ⚠️  ${client.name} — no results from Nominatim`)
        failed++
        await sleep(1100)
        continue
      }
      ...
      if (isNaN(latitude) || isNaN(longitude)) {
        console.log(`[${i + 1}/${clients.length}] ⚠️  ${client.name} — invalid coordinates`)
        failed++
        await sleep(1100)
        continue
      }
      ...
      if (updateError) {
        console.log(`[${i + 1}/${clients.length}] ❌ ${client.name} — ${updateError.message}`)
        failed++
      } else {
        console.log(
