...
async function main(): Promise<void> {
  ...
  for (const row of allRows) {
    ...
    const visitPayload = {
      ...
      status: dayjs(dateOnly).isBefore(dayjs().startOf('day'))
        ? 'completed'
        : 'pending',
    ...
    if (DRY_RUN) {
      ...
      if (error) {
        console.error(`  ✗ Visit "${name}" @ ${scheduledAt}: ${error.message}`);
        errors++;
        continue;
      }

      console.log(
        `  + Visit: ${name} @ ${dayjs(scheduledAt).format('DD/MM/YYYY HH:mm')}`
      );
    }
