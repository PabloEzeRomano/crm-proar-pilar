...
test.describe('Visit Creation (EP-023.4)', () => {
  ...
  test('should display visits list page', async ({ page }) => {
    ...
    const visitsHeader = page.locator('text=/Visitas|Visits/')
  ...
  test('should have button to create new visit', async ({ page }) => {
    ...
    const newVisitButton = page.locator('button:has-text("Nueva visita")')
      .or(page.locator('button:has-text("New Visit")')
  ...
  test('should navigate to visit form when creating new visit', async ({ page }) => {
    // Click new visit button
    const newVisitButton = page.locator('button:has-text("Nueva visita")').first()
    ...
    if (await newVisitButton.isVisible().catch(() => false)) {
      ...
      await page.waitForURL(/visits.*form|\/visits\/new/, { timeout: 5000 }).catch(() => {
        // Navigation might not match expected pattern
      })

      // Verify form elements are visible
      const clientField = page.locator('text=/Cliente|Client/')
        .or(page.locator('input[placeholder*="client" i]'))

      // Form should have some visible elements
      const formContent = page.locator('[role="dialog"], form, [data-testid="visit-form"]')
  ...
  test('should display visit form with required fields', async ({ page }) => {
    ...
    // Client selection
    const clientLabel = page.locator('text=Cliente')
  ...
  test('should show validation error when submitting empty form', async ({ page }) => {
    ...
    if (await saveButton.isVisible().catch(() => false)) {
      ...
      const errorMessage = page.locator('text=/requerido|required|Seleccioná|Choose/')
  ...
  test('should allow filling visit form', async ({ page }) => {
    ...
    const clientInput = page.locator('input[placeholder*="cliente" i]')
...
test.describe('Visit Status Updates (EP-023.4)', () => {
  ...
  test('should navigate to visit detail when clicking a visit', async ({ page }) => {
    // Look for the first visit row and click it
    const visitRow = page.locator('[role="button"]:has-text(/cliente|client/i)').first()
    ...
    if (await visitRow.isVisible().catch(() => false)) {
      ...
      const detailContent = page.locator('text=/Detalles|Detail|Minuta/')
  ...
  test('should show status change options on visit detail', async ({ page }) => {
    // Navigate to a visit detail page
    await page.goto('/visits/1')
    await page.waitForLoadState('networkidle')

    // Look for status update buttons or picker
    const statusButton = page.locator('button:has-text(/Pendiente|Completada|Cancelada/)')
      .or(page.locator('[role="combobox"]:has-text(/status|estado|estado/)')
...
test.describe('Visit Deletion (EP-023.4)', () => {
  ...
  test('should show confirmation before deleting visit', async ({ page }) => {
    ...
    if (await deleteButton.isVisible().catch(() => false)) {
      ...
      const confirmText = page.locator('text=/¿Estás seguro|Are you sure|Confirmar|Delete/')
...
test.describe('Visit List Integration (EP-023.4)', () => {
  ...
  test('should allow filtering by time period (today/week/month)', async ({ page }) => {
    await page.goto('/')
    await page.waitForLoadState('networkidle')
  ...
  test('should show "Todo listo por hoy" when no pending visits', async ({ page }) => {
    await page.goto('/')
    await page.waitForLoadState('networkidle')

    // Look for the "all done" message
    const doneMessage = page.locator('text=Todo listo por hoy')
