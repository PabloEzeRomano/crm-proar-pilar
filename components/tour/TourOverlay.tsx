...
import { useTourStore, TOUR_STEP_ROUTES, TOUR_TOTAL_STEPS } from '@/stores/tourStore'
...
export default function TourOverlay() {
  ...
  const currentStepId = currentIndex !== null ? `step-${currentIndex + 1}` : null
  ...
  useEffect(() => {
    ...
    const prevRoute = prevIndexRef.current !== null ? TOUR_STEP_ROUTES[prevIndexRef.current + 1] : null
  ...
  return (
    <View ref={overlayRef} style={rootStyle} pointerEvents="box-none">
      ...
      {/* Bottom panel */}
      <Pressable
        style={[styles.backdrop, { top: bottomY, left: 0, right: 0, height: bottomH }]}
        onPress={stop}
        accessibilityLabel="Cerrar tour"
      />
      {/* Left panel */}
      <Pressable
        style={[styles.backdrop, { top: hY, left: 0, width: leftW, height: hH }]}
        onPress={stop}
        accessibilityLabel="Cerrar tour"
      />
      {/* Right panel */}
      <Pressable
        style={[styles.backdrop, { top: hY, left: rightX, width: rightW, height: hH }]}
        onPress={stop}
        accessibilityLabel="Cerrar tour"
      />
        ...
        <View
          style={[
            styles.highlight,
            {
              ...
              width: hW,
              height: hH,
              borderRadius: (currentStep?.borderRadius ?? 0) + SPOTLIGHT_PADDING,
            },
          ]}
          pointerEvents="none"
        />
      ...
      {rect && currentStep && (
        <View
          style={[styles.tooltipWrapper, { top: tooltipTop, left: tooltipLeft, maxWidth: TOOLTIP_MAX_W }]}
